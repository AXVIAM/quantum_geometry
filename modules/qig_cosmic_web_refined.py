...
density_field = np.random.rand(size, size)  # Generate a refined random density field

# Define different smoothing scales for refined modeling
...
results_filename = "qig_cosmic_web_refined_results.txt"

with open(results_filename, "w") as file:
    file.write("Quantum Information Gravity - Refined Cosmic Web Structure Results\n")
    file.write("-----------------------------------------------------------\n")
    ...
    for sigma in smoothing_scales:
        ...
        for cmap in color_maps:
            ...
            plt.title(f"Refined Cosmic Web Structure (σ={sigma}, cmap={cmap})")
            plt.xlabel("X Coordinate")
            plt.ylabel("Y Coordinate")

            # Save the figure
            image_filename = f"qig_cosmic_web_refined_sigma_{sigma}_cmap_{cmap}.png"
            plt.savefig(image_filename, dpi=300)
            plt.close()

print(f"✅ Script updated successfully.\nResults saved in {results_filename}\nGenerated multiple refined cosmic web images.")
